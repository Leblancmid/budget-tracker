          ...
          <div className="flex flex-wrap gap-2">
            {COLOR_OPTIONS.map((c) => (
              <button
                ...
                className={['h-7 w-7 rounded-full transition-transform hover:scale-110', form.color === c ? 'ring-2 ring-offset-2 ring-indigo-500 scale-110' : ''].join(' ')}
              />
            ))}
          </div>
        </div>
            ...
            {ICONS.map((ic) => (
              <button
                ...
                className={[
                  ...
                  form.icon === ic ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50',
                ].join(' ')}
              >
                {ic}
              </button>
