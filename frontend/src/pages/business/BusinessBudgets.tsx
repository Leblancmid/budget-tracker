...
import { useBusinessBudgets } from '@/hooks/useBusinessBudgets'
import { useBusinessCategories } from '@/hooks/useBusinessCategories'
import { Button } from '@/components/ui/Button'
import { Select } from '@/components/ui/Select'
import { Card } from '@/components/ui/Card'
import { ConfirmDialog } from '@/components/ui/ConfirmDialog'
import { BusinessBudgetModal } from '@/components/modals/BusinessBudgetModal'
import { toast } from '@/components/ui/Toast'
import { formatCurrency, MONTHS } from '@/utils/format'
import type { BusinessBudget } from '@/types'

export default function BusinessBudgets() {
  const { budgets, month, year, loading, setMonth, setYear, create, update, remove } = useBusinessBudgets()
  const { categories } = useBusinessCategories()

  const [modalOpen, setModalOpen] = useState(false)
  const [editTarget, setEditTarget] = useState<BusinessBudget | null>(null)
  const [deleteTarget, setDeleteTarget] = useState<BusinessBudget | null>(null)
  const [deleteLoading, setDeleteLoading] = useState(false)

  const handleSubmit = async (data: { category_id: number; amount: number; month: number; year: number }) => {
    if (editTarget) {
      await update(editTarget.id, data.amount)
  ...
  const openEdit = (b: BusinessBudget) => { setEditTarget(b); setModalOpen(true) }
        ...
        <Card className="px-5 py-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-right">
              ...
              <p className={['text-xl font-bold', (totalBudget - totalSpent) < 0 ? 'text-red-600 dark:text-red-400' : 'text-teal-600 dark:text-teal-400'].join(' ')}>
          ...
          <div className="h-2 rounded-full bg-gray-100 overflow-hidden dark:bg-gray-700">
            <div
              className={['h-full rounded-full transition-all duration-700', totalSpent > totalBudget ? 'bg-red-500' : 'bg-teal-500'].join(' ')}
              ...
              <Card key={b.id} className="flex flex-col gap-3 p-5">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => openEdit(b)} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-teal-600 transition-colors dark:text-gray-500 dark:hover:bg-gray-700 dark:hover:text-teal-400">
                ...
                <div className="h-2 rounded-full bg-gray-100 overflow-hidden dark:bg-gray-700">
                  <div
                    className={['h-full rounded-full transition-all duration-700', over ? 'bg-red-500' : (b.percentage ?? 0) >= 80 ? 'bg-amber-500' : 'bg-teal-500'].join(' ')}
