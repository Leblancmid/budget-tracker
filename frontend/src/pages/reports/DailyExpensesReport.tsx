...
export default function DailyExpensesReport() {
  ...
  const { data, loading } = useReport('daily-expenses', period, year, month)
  const { data: thisMonth } = useReport('daily-expenses', 'weekly', now.getFullYear(), now.getMonth() + 1)
  ...
  const isPositive = (mTotals?.net ?? 0) >= 0
  const curLabel   = `${MONTHS[now.getMonth()]} ${now.getFullYear()}`

  const handleExport = () => {
    if (!data?.rows.length) return
    exportCsv(`daily-expenses-${period}-${year}`, data.rows.map(r => ({
      period:  r.label,
      income:  r.income,
      expense: r.expense,
      net:     r.net ?? 0,
        ...
        <div className="relative flex flex-col gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <div className="flex h-6 w-6 items-center justify-center rounded-lg bg-indigo-400/20">
                <BarChart2 className="h-3.5 w-3.5 text-indigo-400" />
              </div>
              <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Net Balance</span>
            ...
            {loading ? (
              ...
              <p className={['text-3xl font-bold', isPositive ? 'text-indigo-300' : 'text-red-400'].join(' ')}>
                <Amt value={formatCurrency(mTotals?.net ?? 0)} />
              </p>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {period !== 'yearly' && (
              <select
                ...
                className="rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 px-2.5 py-1.5 text-sm text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-400/40"
              >
            ...
            <button
              ...
              className="hidden md:flex items-center gap-1.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-1.5 text-xs font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-700/60 bg-gray-50 dark:bg-gray-800/40">
                ...
                <th className="px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Net</th>
              </tr>
            </thead>
                  ...
                  return (
                    <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-right font-semibold">
                        <span className={net >= 0 ? 'text-indigo-600 dark:text-indigo-400' : 'text-red-500 dark:text-red-400'}>
                          <Amt value={formatCurrency(net)} />
                        </span>
                      </td>
                    </tr>
                  )
            ...
            {!loading && (data?.rows.length ?? 0) > 0 && (
              <tfoot>
                <tr className="border-t-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/40 font-semibold">
                  ...
                  <td className={['px-5 py-3 text-right text-sm', isPositive ? 'text-indigo-600 dark:text-indigo-400' : 'text-red-500 dark:text-red-400'].join(' ')}>
                    <Amt value={formatCurrency(totals?.net ?? 0)} />
                  </td>
                </tr>
              </tfoot>
            )}
